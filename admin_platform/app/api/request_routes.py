...
def compensatory_leave_form(request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user, use_cache=False)):
    ...
    return render_template("compensatory_leave_form.html", {"request": request, "remaining_hours": remaining_hours, "current_user": current_user})
...
def overtime_page(request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user, use_cache=False)):
    ...
    return render_template("overtime_form.html", {"request": request, "current_user": current_user, "remaining_overtime": remaining_overtime, "max_overtime_hours": max_overtime_hours})
...
def business_trip_form(request: Request, current_user: User = Depends(get_current_user, use_cache=False)):
    return render_template("business_trip_form.html", {"request": request, "current_user": current_user})
...
def self_development_form(request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user, use_cache=False)):
    ...
    return render_template("self_development_form.html", {"request": request, "current_user": current_user, "remaining_dev_cost": remaining_dev_cost})
...
def approve_list(request: Request, db: Session = Depends(get_db), current_user: User = Depends(require_role(["admin", "approver", "lead"])), start_date: str = None, end_date: str = None, selected_name: str = None, request_type: str = None, page: int = 1, status_filter: str = None):
    ...
    query = "SELECT id, name, type, content, status, created, approver, reject_reason, cost, approved_by_lead, approved_by_approver, approved_by_lead_at, approved_by_approver_at, file_path FROM requests WHERE created >= :start_date AND created < :end_date"
...
def edit_request_form(request_id: int, request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user, use_cache=False)):
    ...
    if request_data['type'] == '자기개발비':
        ...
        used_dev_cost = used_dev_cost_result[0] or 0
        remaining_dev_cost = 2000000 - used_dev_cost
        return render_template(template_name, {"request": request, "request_data": request_data, "current_user": current_user, "remaining_dev_cost": remaining_dev_cost})

    print(request_data)
    return render_template(template_name, {"request": request, "request_data": request_data, "current_user": current_user})
